...
export function FixtureBuilderView() {
  const { saveProfile, loadProfiles } = useFixturesStore()
    ...
    <div className="view-full" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem', overflowY: 'auto' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p className="text-muted">Create custom DMX fixture profiles. Saved profiles are instantly available in Patch.</p>
        </div>
        <button 
      ...
      <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
        {/* Info Section */}
        <div style={{ flex: '1', minWidth: '300px', background: 'var(--bg-card)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
        ...
        {/* Channels Section */}
        <div style={{ flex: '2', minWidth: '500px', background: 'var(--bg-card)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
            ...
            {channels.length === 0 ? (
              ...
              channels.map((channel, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--bg-panel)', padding: '0.75rem', borderRadius: 'var(--radius-sm)' }}>
                  ...
                  <div style={{ width: '40px', fontWeight: 'bold', textAlign: 'center', background: 'var(--bg-base)', padding: '0.25rem', borderRadius: '4px' }}>
                    {channel.number}
                  </div>
